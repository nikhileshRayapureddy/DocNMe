...
open class SearchTextField: UITextField {
    ...
    fileprivate func prepareDrawTableResult() {
        guard let frame = self.superview?.convert(self.frame, to: UIApplication.shared.keyWindow) else {
            return
        }
...
public struct SearchTextFieldItem {
    ...
    public init(title: String, subtitle: String?, image: UIImage?, id: String?) {
        self.title = title
        self.subtitle = subtitle
        self.image = image
        self.id = id;
